...
export default function OrderDetailScreen({ order, onBack, themeColors }) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.background }] }>
      ...
      {/* Bọc phần chi tiết trong ScrollView */}
      <ScrollView style={{flex:1}} contentContainerStyle={{paddingBottom: 24}}>
        ...
        {/* Thay FlatList bằng map thường */}
        {order.items && order.items.map((item, idx) => (
          <View key={item.productId + '-' + item.size + '-' + item.color + '-' + idx} style={[styles.productItem, { backgroundColor: themeColors.grayLight }]}> 
            <Image 
              source={
                item.image
